...
function createTask(text, duration = 5, completed = false, realEnd = null, taskToInsertAfter = null)  {
...
// Div pour le contenu principal
  ...
  subtaskInput.placeholder = "Nom de la sous-tâche...";

  const addSubtaskConfirmBtn = document.createElement("button");
  addSubtaskConfirmBtn.textContent = "Ajouter";
  addSubtaskConfirmBtn.classList.add("add-subtask-confirm-btn");
  ...
  subtaskInput.addEventListener("keypress", e => { if (e.key === "Enter") {
	  ...
	  createTask(subtaskInput.value, duration, false, null,taskDiv);  // Crée la tâche avec le texte et la durée calculée
	  subtaskInput.value = "";  // Réinitialise le champ de saisie
	  saveTasks();  // Sauvegarde les tâches
  }});

  // Lorsque le bouton "Ajouter" est cliqué
  addSubtaskConfirmBtn.addEventListener("click", () => {
	  const text = subtaskInput.value.trim();  // Récupère le texte sans espaces avant et après
	  let duration =  5; // Valeur par défaut à 5 min
  ...
  subtaskInputContainer.append(subtaskInput, addSubtaskConfirmBtn);
...
// Regrouper les éléments
  ...
  durationDiv.append(slider, durationSpan, timeSpan, addAfterBtn, subtaskInputContainer);
