...
// Div pour le contenu principal
  ...
  subtaskInput.addEventListener("keydown", e => { 
	if (e.key === "Enter") {
	  ...
	  const tasks = subtaskInput.value.split(",").reverse();

	  tasks.forEach((part, index) => {
		let text = part.trim(); 
	  ...
	  if (durationMatch) {
		...
		text = text.replace(durationMatch[0], '').trim();  // Enlève la durée du texte
	  }

	  // Si le texte est vide après le nettoyage, on ne crée pas la tâche
	  if (!text.trim()) return;

	  createTask(text, duration, false, null,taskDiv,subtaskInputContainer.dataset.direction);  // Crée la tâche avec le texte et la durée calculée
	  })
...
function addTask() {
  ...
  tasks.forEach((part, index) => {
    ...
    createTask(text, duration, false, null);  // Crée la tâche avec le texte et la durée calculée
  });
